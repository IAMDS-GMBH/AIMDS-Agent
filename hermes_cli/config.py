...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # Values: true (default — applies to all models), "auto" (model-family
        # based), false
    ...
    "curator": {
        ...
        # period, not just agent-created ones. OFF by default: the shipped
        # skills are the product surface and come back on every `hermes
        # update` anyway, so pruning them only "sticks" through a suppression
        # list that fights the re-seeder — and the LLM curation pass would
        # otherwise archive/absorb freshly shipped skills before anyone used
        # them. Hub-installed skills are NEVER pruned here — they have an
        # external upstream owner. When switched on, built-ins accrue usage
        # telemetry and their inactivity clock starts the first time the
        # curator sees them (never a mass-prune on the first run).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # ── Version 32 → 33 (+ idempotent): canonicalize IAMDS memory_upsert filters ──
    # Target only MCP servers with provider='iamds' (name-agnostic policy).
    # Runs as a normal versioned migration and remains idempotent for already-
    # migrated configs (including users already at _config_version 33).
    try:
        config = read_raw_config()
        mcp_servers = config.get("mcp_servers")
        touched = False
        ...
        if touched:
            ...
            save_config(config)
            results["config_added"].append(
                f"iamds mcp tool filters canonicalized ({renamed_count} memory_upsert→memory_save rename(s))"
            )
            if not quiet:
                print(
                    f"  ✓ Canonicalized IAMDS MCP tool filters: renamed {renamed_count} "

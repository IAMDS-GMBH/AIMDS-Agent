...
export const zh: Translations = {
  ...
  settings: {
    ...
    gateway: {
      ...
      openLogs: '打开日志',
      localConnTitle: '本地连接',
      remoteConnTitle: '远程连接',
      statusHealthy: '状态：正常',
      statusUnknown: '状态：未知',
      statusError: (msg: string) => `状态：${msg}`,
      endpointLocal: (url: string) => `端点：Hermes 本地 API (${url})`,
      endpointNotConfigured: '端点：未配置',
      endpointRemote: (url: string) => `端点：${url}`,
      checkedAt: (time: string) => `检查时间：${time}`,
      checking: '正在检查…',
      refresh: '刷新',
      runtimeChecks: '运行时检查',
      hermesLocalApi: 'Hermes 本地 API',
      statusUp: '运行中',
      gatewayProcess: '网关进程',
      gatewayOptional: '可选（未运行）',
      dashboardAuthGate: 'Dashboard 认证',
      authGateEnabled: (providers: string) => `已启用 (${providers})`,
      authGateDisabled: '已禁用',
      configuredMcp: '已配置的 MCP 服务器',
      criticalServices: '关键服务',
      remoteHealthFailed: '远程健康检查失败',
      supportTitle: '支持',
      sendSupportLogs: '发送支持日志',
      sendingSupportLogs: '正在发送支持日志…',
      supportNotConfigured: '支持日志尚未配置。请在下方设置上传 URL 和 API 密钥。',
      supportLogSettings: '支持日志上传设置',
      supportUploadUrlPlaceholder: 'https://support.example.com/api/log-upload',
      apiKeyPlaceholder: 'API 密钥',
      apiKeySetPlaceholder: 'API 密钥已设置（输入以替换）',
      apiKeyConfigured: 'API 密钥当前已配置。',
      apiKeyNotConfigured: 'API 密钥未配置。',
      saveSupportSettings: '保存支持设置',
      savingLabel: '保存中…',

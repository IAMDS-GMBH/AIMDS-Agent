...
- Target hours depend on country/federal state/canton, weekly hours, working days (5/6-day weeks or explicit `work_weekdays` like Mo–We), full/part time and half days (24 and 31 December). The profile lives in memory as `Arbeitszeit-Profil`; `workdays(action='profile')` shows it. It also carries the genericity keys: `worklog_source_tool` (LIKE pattern(s) matching the user's time bookings in `mcp_records.tool_name` — any worklog MCP, several combine, e.g. Tempo history plus OpenProject time entries), `worklog_user` (whose rows count when a source holds the whole team's entries) and optionally `vacation_booking_patterns` / `special_leave_booking_patterns` / `sick_booking_patterns` + `vacation_hour_factor` for absences booked as time entries. The patterns match the booked item's key **or title** (`mcp_records.title`), so a year-neutral title pattern such as `INTERNAL_URLAUB_%` survives the yearly new ticket and a move between ticket systems.
- If `workdays` answers `worktime profile unknown`: **first try `workdays(action='estimate_profile')`** — it proposes a week model, every booking source, the booking user (only when unambiguous — otherwise it asks) and absence patterns derived from the booked items' titles. Present the proposal and confirm via `clarify` (region is never estimated; also confirm weekly hours and full/part time), then `workdays(action='configure', region=…, weekly_hours=…, days_per_week=… or work_weekdays=[…], half_days=[…], worklog_source_tool=…, …)` — this saves to memory. Never assume BW/DE or 40 h / 5 days.
...
- For actual-vs-target questions call `workdays(action='report', …)` first: it materializes the calendar, aggregates actuals from `mcp_records` via the profile's `worklog_source_tool`, refreshes absences booked as time entries, credits every absence kind (vacation, special leave, sick) from `absences`, counts presence days, and computes delta — all in SQLite, clamped to today. The tool description lists what the report can do (relative periods, per-day rows with first start / last end, the rendered vault file); its `hints` say what is missing or stale. Use those instead of hand-written date arithmetic or a self-written report file.
...
1. **Booked absences:** bookings matching the profile's `vacation_/special_leave_/sick_booking_patterns` (key or title) become day portions via `vacation_hour_factor` (e.g. factor 8.0: 1 h booked = a full 8 h day). `report` refreshes them for its range by itself; `workdays(action='absences', op='import_from_bookings')` does it explicitly. Days the user, a vault note or a document recorded are never overwritten.
...
6. **Never guess municipal holidays:** Whether a partial holiday applies (Augsburg? catholic municipality?) is asked ONCE and stored in `partial_holidays` — never assumed from the state alone, never silently deducted.

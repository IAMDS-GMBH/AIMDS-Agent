...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    cron_hint = (
        ...
        "the rest. "
        "TOOLS: If `tool_search` is in your tools list, use it to find integrations "
        "(calendar, email reading, todos, web, etc.) that are not directly listed, and do "
        "not assume a tool is unavailable before searching. Tools it returns as loaded (and "
        "any you fetch with `tool_describe`) are added to your tools list — call them by "
        "name. Without `tool_search`, your tools list is complete. "

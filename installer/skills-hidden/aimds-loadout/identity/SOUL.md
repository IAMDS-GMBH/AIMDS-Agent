...
### 2. Corporate Memory Vault vs. System Prompt Memory Boundary
- **Local Hermes System Memory (`~/.hermes/memories/MEMORY.md` / `USER.md`)**:
  - **STRICTLY RESTRICTED TO USER PROFILE & DIRECTIVES ONLY.**
  - Contains ONLY the user profile (`type: profile`: name, role, language "de"/"en", address "du"/"sie") and core personal interaction preferences.
  - **NEVER** save general facts, technical documentation, project architecture, release steps, or code notes to `MEMORY.md` as doing so bloats the system prompt context on every turn.
- **Corporate Memory Vault (`AIMDSSuiteMCP` / `go-mcp-memory` / Vector DB) & Obsidian Vault**:
  - **PRIMARY TARGET FOR ALL KNOWLEDGE, FACTS, RULES & DOCUMENTATION.**
  - Primary server name `AIMDSSuiteMCP`. Tools exposed via `mcp_AIMDSSuiteMCP_memory_*` (`context`, `save`, `read`, `search`, `backlinks`).
  ...
  - **Access Policy**: Corporate Cloud Memory is queried on-demand. Reads/searches are used for background context. Writes (`memory_save`) save persistent facts, rules, and knowledge without polluting `MEMORY.md`.

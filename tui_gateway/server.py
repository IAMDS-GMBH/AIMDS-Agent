...
def _completion_cwd(params: dict | None = None) -> str:
    ...
    try:
        ...
        if any(pat in raw_str for pat in legacy_patterns) and "Documents/AIMDS-Suite-Vault" not in raw_str:
...
def _session_cwd(session: dict | None) -> str:
    if session and session.get("cwd"):
        ...
        if any(pat in cwd_str for pat in legacy_patterns) and "Documents/AIMDS-Suite-Vault" not in cwd_str:
            from hermes_cli.config import _resolve_workspace_dir
...
def _apply_model_switch(
    ...
    *,
    ...
    # (prod/staging/dev/local), a 3rd-party model → prod. Reconnects only when
    # the host changes.
    try:
...
def _tool_summary(name: str, result: str, duration_s: float | None) -> str | None:
    ...
    if name == "web_search" and isinstance(data, dict):
        ...
        # Keep memory_context summaries compact: raw payloads can contain nested
        # JSON (including onboarding prompt artifacts) that should not surface in
        # the user-visible transcript/tool row.
        text = (
            "Memory context loaded; onboarding started"
            if data.get("onboarding_init_auto_started")

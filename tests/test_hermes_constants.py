...
class TestHermesManagedNode:
    ...
    def test_windows_path_skips_permission_error_and_falls_back_to_which(
        self, tmp_path, monkeypatch
    ):
        """PermissionError on stat (e.g. nvm4w junction) must not crash; falls back to shutil.which."""
        from pathlib import Path
        from unittest.mock import patch

        # Simulate a PATH with one inaccessible directory (raises PermissionError on is_file)
        # followed by nothing; shutil.which should still find npm.
        inaccessible_dir = str(tmp_path / "nvm4w" / "nodejs")
        good_npm = tmp_path / "fallback" / "npm.cmd"
        good_npm.parent.mkdir(parents=True)
        good_npm.write_text("@echo off\n")

        monkeypatch.setattr(hermes_constants.sys, "platform", "win32")
        monkeypatch.setenv("PATH", inaccessible_dir)

        original_is_file = Path.is_file

        def is_file_raising(self):
            if str(self).startswith(inaccessible_dir):
                raise PermissionError(5, "Access is denied", str(self))
            return original_is_file(self)

        with patch.object(Path, "is_file", is_file_raising):
            with patch("shutil.which", return_value=str(good_npm)):
                result = find_node_executable_on_path("npm")

        assert result == str(good_npm)

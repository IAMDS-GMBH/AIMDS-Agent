...
class TestAimdsSuiteLitellm:
    ...
    Hermes-agent owns the cache breakpoints for AIMDS-Suite endpoints (OpenAI-wire
    layout): the proxy's prompt_cache_hook only ever marked `system` and the last
    tool, so the conversation was re-sent uncached every turn (~30 % hit rate).
    The hook keeps client markers untouched, so the 4-block budget is ours.
    """

    def test_aimds_suite_prod_caches_client_side_openai_layout(self):
        agent = _make_agent(
    ...
    def test_aimds_suite_staging_caches_client_side_openai_layout(self):
        agent = _make_agent(
    ...
    def test_aimds_suite_dev_caches_client_side_openai_layout(self):
        agent = _make_agent(
    ...
    def test_iamds_litellm_alias_caches_client_side_openai_layout(self):
        agent = _make_agent(

...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # ``office`` and ``ticket_routing`` ride along: hermes-cli includes them
    # (AIS-294, AIS-327) and _get_platform_tools recovers non-configurable
    # platform toolsets.
    assert server._load_enabled_toolsets() == ["memory", "office", "ticket_routing"]
...
def test_litellm_hub_install_accepts_object_source_with_url_and_path(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir()
    token = set_hermes_home_override(home)
    try:
...
"""
        ...
        def _fake_get(url, *args, **kwargs):
            ...
            if url == "https://api.github.com/repos/owner/repo/git/trees/main":
                return _Resp(
                    200,
                    payload={"tree": [{"type": "blob", "path": "skills/grill-me/SKILL.md"}]},
                )
            if url.endswith("/skills/grill-me/SKILL.md"):
                return _Resp(200, text=skill_md)
            return _Resp(404, text="")
        ...
        hint_seen = {"value": None}

        def _fake_find(owner, repo, skill_id_hint=None, plugin_name=None):
            hint_seen["value"] = skill_id_hint
            return (
                "https://raw.githubusercontent.com/owner/repo/main/skills/grill-me/SKILL.md",
                "skills/grill-me/SKILL.md",
            )

        monkeypatch.setattr(hub, "find_skill_md_in_repo", _fake_find)
        monkeypatch.setattr(
            hub, "GitHubAuth", lambda: types.SimpleNamespace(get_headers=lambda: {})
        )
        monkeypatch.setattr(hub, "SKILLS_DIR", home / "skills")
        ...
        result = resp["result"]
        assert result["success"] is True
        assert (home / "skills" / "from_skill_hub" / "grill-me" / "SKILL.md").exists()
        assert lock.entries["grill-me"]["install_path"] == "from_skill_hub/grill-me"
        assert hint_seen["value"] == "grill-me"
    finally:
        reset_hermes_home_override(token)
...
def test_litellm_hub_install_division_set_conflict_409_is_warning(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir()
    token = set_hermes_home_override(home)
    try:
            ...
            if url == "https://api.github.com/repos/owner/repo/contents/engineering":
                return _Resp(
                    ...
                    payload=[
                        ...
                        {
                            "type": "file",
                            "name": "engineering-conflict.md",
                            "path": "engineering/engineering-conflict.md",
                            "download_url": "https://raw.githubusercontent.com/owner/repo/main/engineering/engineering-conflict.md",
                        },
                    ],
                )
            if url.endswith("/engineering/engineering-architect.md"):
                return _Resp(200, text=ok_md)
            if url.endswith("/engineering/engineering-conflict.md"):
        ...
        assert result["installed_skills"] == ["engineering-architect"]
        assert len(result["conflict_skills"]) == 1
    finally:
        reset_hermes_home_override(token)


def test_litellm_hub_install_division_set_rate_limit_429_continues_as_warning(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir()
    token = set_hermes_home_override(home)
    try:
            ...
            if url == "https://api.github.com/repos/owner/repo/contents/engineering":
                return _Resp(
                    ...
                    payload=[
                        ...
                        {
                            "type": "file",
                            "name": "engineering-busy.md",
                            "path": "engineering/engineering-busy.md",
                            "download_url": "https://raw.githubusercontent.com/owner/repo/main/engineering/engineering-busy.md",
                        },
                    ],
                )
            if url.endswith("/engineering/engineering-architect.md"):
                return _Resp(200, text=ok_md)
            if url.endswith("/engineering/engineering-busy.md"):
                return _Resp(429, text="Too Many Requests")
            return _Resp(404, text="")
        ...
        assert result["warning"] is True
        assert result["installed_skills"] == ["engineering-architect"]
        assert result["failed_skills"] == ["engineering/engineering-busy.md"]
    finally:
        reset_hermes_home_override(token)


def test_litellm_hub_install_division_set_emits_progress_from_zero(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir()
    token = set_hermes_home_override(home)
    try:
        ...
        monkeypatch.setattr(server, "_emit", lambda event, sid, payload=None: emitted.append((event, sid, payload or {})))
        ...
        resp = server._methods["litellm_hub.skill_install"](
            "litellm-install",
            {
                ...
                "install_run_id": "run-123",
                "session_id": "session-1",
            },
        )

        assert "result" in resp
        progress = [e for e in emitted if e[0] == "litellm_hub.skill_install.progress"]
        assert len(progress) >= 4
        assert progress[0][1] == "session-1"
        assert progress[0][2]["install_run_id"] == "run-123"
        assert progress[0][2]["completed"] == 0
        assert progress[0][2]["total"] == 2
        assert progress[-1][2]["status"] == "complete"
        assert progress[-1][2]["completed"] == 2
        assert progress[-1][2]["conflicts"] == 1
    finally:
        reset_hermes_home_override(token)


def test_litellm_hub_skill_uninstall_falls_back_to_skill_id_set_cleanup(monkeypatch):
    import tools.skills_hub as hub

    class _Lock:
        def list_installed(self):
            return [
                {"name": "engineering", "metadata": {"skill_id": "set-123"}},
                {"name": "engineering-architect", "metadata": {"skill_id": "set-123", "dependency_of": "engineering"}},
                {"name": "engineering-qa-lead", "metadata": {"skill_id": "set-123", "dependency_of": "engineering"}},
            ]

    removed: list[str] = []

    def _fake_uninstall(name: str):
        if name == "engineering-architect":
            removed.append(name)
            return True, "ok"
        if name == "engineering-qa-lead":
            removed.append(name)
            return True, "ok"
        if name == "engineering":
            return False, "'engineering' is not a hub-installed skill (may be a builtin)"
    ...
    result = resp["result"]
    assert result["success"] is True
    assert set(result["removed_skills"]) == {"engineering-architect", "engineering-qa-lead"}
    assert set(removed) == {"engineering-architect", "engineering-qa-lead"}
...
def test_interim_assistant_callback_emits_message_delta_when_not_streamed():
    with patch("tui_gateway.server._emit") as emit:
        cb = server._agent_cbs("sid")["interim_assistant_callback"]
        cb("Profile is empty; onboarding will start now.", already_streamed=False)

    emit.assert_called_once_with(
        "message.delta",
        "sid",
        {"text": "Profile is empty; onboarding will start now.", "interim": True},
    )


def test_interim_assistant_callback_skips_when_already_streamed():
    with patch("tui_gateway.server._emit") as emit:
        cb = server._agent_cbs("sid")["interim_assistant_callback"]
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / "documents" / "attachments" / today / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == f"@file:documents/attachments/{today}/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        today = datetime.now().date().isoformat()
        stored = workspace / "documents" / "attachments" / today / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == f"@file:documents/attachments/{today}/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        # No copy: nothing staged under documents/attachments.
        assert not (workspace / "documents" / "attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        today = datetime.now().date().isoformat()
        assert resp["result"]["ref_text"] == f"@file:`documents/attachments/{today}/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_prompt_submit_persists_display_text_in_history_when_provided(monkeypatch):
    """When display_text is present (desktop slash skill invocation), store that
    user text in session history instead of the expanded internal payload."""
        ...
        def run_conversation(
            ...
            return {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": '[IMPORTANT: The user has invoked the "grill-me" skill, indicating they want you to follow its instructions. The full skill content is loaded below.]',
                    },
                    {"role": "assistant", "content": "Done."},
                ],
            }

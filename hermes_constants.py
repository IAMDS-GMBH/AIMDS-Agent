...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    if has_path_separator:
        try:
            is_file = Path(command_str).is_file()
        except OSError:
            is_file = False
        return command_str if is_file else None
    ...
    for name in _candidate_node_command_names(command_str):
        for directory in os.environ.get("PATH", "").split(os.pathsep):
            ...
            try:
                ...
                continue
    # Fall back to shutil.which so junction-based installs (e.g. nvm4w) that
    # raise PermissionError on stat() are still found.
    return shutil.which(command_str)

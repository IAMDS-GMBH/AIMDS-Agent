...
def profile_build_directive() -> str:
    ...
    return (
        ...
        "Also offer (optional) the work-time profile — country and state/canton, hours per week, days per "
        "week, half days — and if given, store it with `workdays(action='configure', region=…, weekly_hours=…, "
        "days_per_week=…, half_days=[…])` instead of a free-text memory note. Volunteered/pre-loaded facts come first.\n"

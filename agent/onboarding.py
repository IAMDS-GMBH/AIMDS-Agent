...
def profile_build_directive() -> str:
    ...
    return (
        ...
        "ONLY IF they accept:\n"
        "  0. First check if AIMDSSuiteMCP or corporate memory tools (`memory_read`, "
        "`memory_search`, `kb_search`, `memory_context`) are available. If available, "
        "query existing user context/memory first to infer known user role, preferences, "
        "and project facts automatically — saving the user from repeating already known info.\n"
        "  1. Ask, in the user's own language, for whatever they're comfortable sharing or confirming: "
        "name, what they do, how they like you to work, and their preferred answer language. "
        "Also offer (optional) the work-time profile — country and state/canton, hours per week (40h? 20h?), "
        "working days (Mo-Fr, Mo-Sa, or explicit days like Mo-We), full or part time, half days, and — when the "
        "state has municipal holidays (e.g. Bavaria's Augsburger Friedensfest, city of Augsburg only) — whether "
        "they apply at the user's location (city/PLZ helps) — and if given, "
        "store it with `workdays(action='configure', region=…, weekly_hours=…, days_per_week=… or work_weekdays=[…], "
        "half_days=[…])` instead of a free-text memory note. If worklog data is already ingested, "
        "`workdays(action='estimate_profile')` proposes a week model from it — present the estimate and ask the "
        "user to confirm it before saving. Volunteered/pre-loaded facts come first.\n"
        "  2. Before ANY external account lookup, say what you intend to look up and "
        "get explicit consent for that step. Never read their connected accounts "
        "(email, calendar, etc.) silently — ask each time.\n"
...
def mark_seen(config_path: Path, flag: str) -> bool:
    ...
    Uses atomic roundtrip YAML update so comments and formatting in config.yaml
    are preserved intact. Returns True on success, False on any error.
    """
    try:

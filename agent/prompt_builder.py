    ...
    *demanded* the `sql` tool — in sessions that had no `sql` tool (every
    ...
    if has_workdays:
        rungs.append(
            ...
            "math in SQLite; per-day start/end/hours come from `include_days=true`, relative ranges (\"bis Ende "
            "letzter Woche\", month to date) from `period=…`, and the Markdown report file from `write='vault'` "
            ...
            "(`workdays(action='absences')`: booking import, direct user input, vault notes, documents); office vs. "
            "home-office days from `workdays(action='presence')` (calendar import with the user's name patterns). "
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "- If a tool returns empty or partial results (or fails to find expected items), do NOT repeat previous cached text — retry immediately with a different query, parameters, or alternative tool strategy.\n"
    ...
    "The 'Current Local Time & Date' block in this prompt is the single source "
    "of truth for today. Dates in conversation history, compaction summaries, "
    "and memory may be older — always recompute relative expressions (today, "
    "tomorrow, next week, calendar-week numbers) against the current date "
    "block, never against a date mentioned earlier in the conversation.\n"
    "</mandatory_tool_use>\n"
    "\n"
    "<act_dont_ask>\n"
...
def build_remote_mcp_memory_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    return (
        ...
        "global rules `always` FIRST so they survive the filter), and stale/duplicate entries can be cleaned "
        "with the memory manage tools. Only act after the user confirms; never silently filter or delete.\n"
        f"{close_hint}"
        f"{onboarding_hint}"

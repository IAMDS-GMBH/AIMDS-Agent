...
def build_remote_mcp_memory_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    return (
        ...
        f"{workspace_line}"
        f"- **Cross-device AIMDSSuiteMCP Memory Vault (`{tool_name}`, narrower/opt-in)**: Central synchronized store across devices and WebUI interfaces (exclusive primary server: `AIMDSSuiteMCP`). Prefer local storage UNLESS the information specifically must persist and sync across ALL sessions/devices — only then call `{tool_name}` or memory search tools (durable user profile facts, standing rules/preferences, contacts, cross-device context). Do not default to it for session-specific or working detail; that stays local.\n"
